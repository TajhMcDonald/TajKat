    ...
    func decorateItems(_ chatItems: [ChatItemProtocol]) -> [DecoratedChatItem]{
        ...
        for (index, item) in chatItems.enumerated(){
            ...
            if addTimestamp == true {
                let timeSeparatorModel = TimeSeparatorModel(uid: UUID().uuidString, date: (item as! MessageModelProtocol).date.toWeekDayAndDateString())
                decoratedItems.append(DecoratedChatItem(chatItem: timeSeparatorModel, decorationAttributes: nil))
            }
            let bottomMargin = separationAfterItem(current: item, next: nextMessage)
            
            let decoratedItem = DecoratedChatItem(chatItem: item, decorationAttributes: ChatItemDecorationAttributes(bottomMargin: 3, showsTail: showsTail, canShowAvatar: showsTail))
